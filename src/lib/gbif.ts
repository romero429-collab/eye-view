...
export function occurrenceFacts(
  ...
  return [
    ...
    rec.kingdom ? { label: "Kingdom", value: rec.kingdom } : null,
    rec.phylum ? { label: "Phylum", value: rec.phylum } : null,
    rec.class ? { label: "Class", value: rec.class } : null,
    rec.order ? { label: "Order", value: rec.order } : null,
    rec.family ? { label: "Family", value: rec.family } : null,
    rec.genericName ? { label: "Genus", value: rec.genericName } : null,
    rec.taxonKey != null ? { label: "Taxon key", value: String(rec.taxonKey) } : null,
