...
4. **Zone memory** — walk or query to tag / reclass / split / merge. The look-at **commits immediately** into the rule engine. Connected blocks receive a **queued** proposal (one hop). They stay independent until you confirm — a change never silently overwrites the next district.
   Click, color, and the inspector all go through the same `resolveZone` posterior: an OSM industrial cell you just taught as residential **paints and inspects as residential**. Dashed amber is queued; solid fill is applied. The hierarchy header becomes “This tick” so the walk feeds back in the same frame, not a later batch.
5. **Events in districts** — hazards dim and coordinate with the zone, they do not replace it.
6. **Plants as structure** — vegetation is a queryable overlay (NDVI + GBIF + iNaturalist + OSM cover), not scenery. In walk mode, woods extrude and named trees stand up on the DEM.
